...
# === Get weights/bias from first FC layer ===
# Layer: Linear(50176 -> 128)
...
enc_input = ts.ckks_vector(context, features)
print("🔒 Feature vector encrypted successfully.")

# === Compute encrypted outputs for FC1 layer ===
enc_outputs = []
for i in range(fc1_weight.shape[0]):
    w_i = fc1_weight[i].tolist()     # ensure it's a list of floats
    b_i = float(fc1_bias[i])

    # Encrypted × Plaintext dot product (supported in TenSEAL 0.2.x)
    enc_result = enc_input.dot(w_i)

    # Decrypt and add bias
    decrypted_val = enc_result.decrypt()[0]
    final_val = decrypted_val + b_i
    enc_outputs.append(final_val)

enc_outputs = np.array(enc_outputs)
print(f"🔓 Decrypted encrypted layer-1 outputs shape: {enc_outputs.shape}")


# === Compute plain outputs from the same FC1 layer ===
with torch.no_grad():
    plain_fc1_output = model.fc[0](torch.tensor(features).unsqueeze(0)).squeeze().numpy()

# === Compare plain vs encrypted ===
mse = np.mean((enc_outputs - plain_fc1_output) ** 2)
corr = np.corrcoef(enc_outputs, plain_fc1_output)[0, 1]

print("\n✅ Encrypted layer-1 inference complete!")
