...
print(f"⏱️ Encryption time: {time.time() - start_enc:.2f} sec\n")
...
for i in range(fc1_weight.shape[0]):
    ...
    for j in range(num_chunks):
        ...
        total_val += enc_dot.decrypt()[0]  # decrypt partial result and sum
...
print(f"⏱️ Encrypted inference time: {enc_time:.2f} sec\n")

# === Compute plain outputs for same layer ===
with torch.no_grad():
...
print(f"⚙️ Total runtime: {time.time() - start_enc:.2f} sec")
